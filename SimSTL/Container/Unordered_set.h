...
namespace SimSTL
{
	...
	namespace Container 
	{
		...
		class KeyEqual = SimSTL::equal_to<Key>, class Allocator = SimSTL::allocator < Key >>
	...
	private:
		template<class Key, class ListIterator, class Hash, class KeyEqual, class Allocator>
		friend class Container::ust_iterator;
	public:
		...
		typedef typename SimSTL::list<key_type>::iterator local_iterator;
		typedef Container::ust_iterator<Key, typename SimSTL::list<key_type>::iterator, Hash, KeyEqual, Allocator> iterator;
	private:
		SimSTL::vector<SimSTL::list<key_type>> buckets_;
		size_type size_;
...
#define PRIME_LIST_SIZE 28
	...
	public:
		...
		SimSTL::pair<iterator, bool> insert(const value_type& val);
		template<class InputIterator>

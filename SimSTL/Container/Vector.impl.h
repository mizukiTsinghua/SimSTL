...
namespace SimSTL{
	...
	void vector<T, Alloc>::resize(size_type n, value_type val = value_type()){
		if (n < size()){
			...
			finish_ = SimSTL::uninitialized_fill_n(finish_, lengthOfInsert, val);
		}
		else if (n > capacity()){
			auto lengthOfInsert = n - size();
			T *newStart = dataAllocator::allocate(getNewCapacity(lengthOfInsert));
			T *newFinish = SimSTL::uninitialized_copy(begin(), end(), newStart);
			newFinish = SimSTL::uninitialized_fill_n(newFinish, lengthOfInsert, val);
	...
	void vector<T, Alloc>::reallocateAndCopy(iterator position, InputIterator first, InputIterator last){
		...
		T *newFinish = SimSTL::uninitialized_copy(begin(), position, newStart);
		newFinish = SimSTL::uninitialized_copy(first, last, newFinish);
		newFinish = SimSTL::uninitialized_copy(position, end(), newFinish);
	...
	void vector<T, Alloc>::reallocateAndFillN(iterator position, const size_type& n, const value_type& val){
		...
		T *newEndOfStorage = newStart + newCapacity;
		T *newFinish = SimSTL::uninitialized_copy(begin(), position, newStart);
		newFinish = SimSTL::uninitialized_fill_n(newFinish, n, val);
		newFinish = SimSTL::uninitialized_copy(position, end(), newFinish);
	...
	void vector<T, Alloc>::insert_aux(iterator position,
		...
		if (locationLeft >= locationNeed){
			if (finish_ - position > locationNeed){
				SimSTL::uninitialized_copy(finish_ - locationNeed, finish_, finish_);
				std::copy_backward(position, finish_ - locationNeed, finish_);
				std::copy(first, last, position);
			}
			else{
				iterator temp = SimSTL::uninitialized_copy(first + (finish_ - position), last, finish_);
				SimSTL::uninitialized_copy(position, finish_, temp);
	...
	void vector<T, Alloc>::insert_aux(iterator position, Integer n, const value_type& value, std::true_type){
		...
		if (locationLeft >= locationNeed){
			...
			SimSTL::uninitialized_fill_n(position, n, value);
			finish_ += locationNeed;
		}
		else{
	...
	void vector<T, Alloc>::shrink_to_fit(){
		...
		finish_ = SimSTL::uninitialized_copy(start_, finish_, t);
	...
	void vector<T, Alloc>::swap(vector& v){
		if (this != &v){
			SimSTL::swap(start_, v.start_);
			SimSTL::swap(finish_, v.finish_);
			SimSTL::swap(endOfStorage_, v.endOfStorage_);
		}
	}
	template<class T, class Alloc>
	...
	void vector<T, Alloc>::allocateAndFillN(const size_type n, const value_type& value){
		...
		SimSTL::uninitialized_fill_n(start_, n, value);
	...
	void vector<T, Alloc>::allocateAndCopy(InputIterator first, InputIterator last){
		...
		finish_ = SimSTL::uninitialized_copy(first, last, start_);
		endOfStorage_ = finish_;
	}
	template<class T, class Alloc>
	template<class InputIterator>
	...
	typename vector<T, Alloc>::size_type vector<T, Alloc>::getNewCapacity(size_type len)const{
		...
		auto res = SimSTL::max(oldCapacity, len);

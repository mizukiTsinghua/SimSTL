...
namespace SimSTL{
	namespace Container{
		...
		void dq_iter<T>::swap(dq_iter& it){
			SimSTL::swap(mapIndex_, it.mapIndex_);
			SimSTL::swap(cur_, it.cur_);
			//SimSTL::swap(container_, it.container_);
		}
		template<class T>
	...
	void deque<T, Alloc>::push_back(const value_type& val){
		...
		//bug fix
		//2015.01.02
		SimSTL::construct(end_.cur_, val);
		++end_;
	}
	template<class T, class Alloc>
	void deque<T, Alloc>::push_front(const value_type& val){
		...
		//bug fix
		//2015.01.02
		SimSTL::construct(beg_.cur_, val);
	}
	template<class T, class Alloc>
	...
	void deque<T, Alloc>::swap(deque<T, Alloc>& x){
		SimSTL::swap(mapSize_, x.mapSize_);
		SimSTL::swap(map_, x.map_);
		beg_.swap(x.beg_);
		end_.swap(x.end_);
	}

...
namespace SimSTL{
	...
	void string::resize(size_t n, char c){
		if (n < size()){
			...
			finish_ = SimSTL::uninitialized_fill_n(finish_, lengthOfInsert, c);
		}
		else if (n > capacity()){
			auto lengthOfInsert = n - size();
			iterator newStart = dataAllocator::allocate(getNewCapacity(lengthOfInsert));
			iterator newFinish = SimSTL::uninitialized_copy(begin(), end(), newStart);
			newFinish = SimSTL::uninitialized_fill_n(newFinish, lengthOfInsert, c);
	...
	string::iterator string::insert_aux_filln(iterator p, size_t n, value_type c){
		...
		newFinish = SimSTL::uninitialized_fill_n(newFinish, n, c);
		auto res = newFinish;
	...
	string::iterator string::insert(iterator p, size_t n, char c){
		...
		if (n <= lengthOfLeft){
			...
			SimSTL::uninitialized_fill_n(p, n, c);
			finish_ += n;
			return (p + n);
		}
		else{
	...
	void string::allocateAndFillN(size_t n, char c){
		...
		finish_ = SimSTL::uninitialized_fill_n(start_, n, c);

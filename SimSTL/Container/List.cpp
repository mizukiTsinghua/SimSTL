...
namespace SimSTL{
	...
	typename list<T>::nodePtr list<T>::newNode(const T& val = T()){
		...
		nodeAllocator::construct(res, Container::node<T>(val, nullptr, nullptr, this));
		return res;
	}
	template<class T>
	...
	typename list<T>::const_iterator list<T>::changeIteratorToConstIterator(iterator& it)const{
		using nodeP = Container::node<const T>*;
		auto temp = (list<const T>*const)this;
		auto ptr = it.p;
		Container::node<const T> node(ptr->data, (nodeP)(ptr->prev), (nodeP)(ptr->next), temp);
	...
	void list<T>::swap(list& x){
		SimSTL::swap(head.p, x.head.p);
		SimSTL::swap(tail.p, x.tail.p);
	}
	template<class T>

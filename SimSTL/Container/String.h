...
#include "Adapters/ReverseIterator.h"
#include "Library/UninitializedFunctions.h"
...
namespace SimSTL{
	...
	private:
		...
		typedef SimSTL::allocator<char> dataAllocator;
	public:
		...
		void swap(string& str){
			SimSTL::swap(start_, str.start_);
			SimSTL::swap(finish_, str.finish_);
			SimSTL::swap(endOfStorage_, str.endOfStorage_);
		}
		size_t copy(char* s, size_t len, size_t pos = 0) const{
			auto ptr = SimSTL::uninitialized_copy(begin() + pos, begin() + pos + len, s);
			return (size_t)(ptr - s);
		}
	...
	string::iterator string::insert_aux_copy(iterator p, InputIterator first, InputIterator last){
		...
		iterator newFinish = SimSTL::uninitialized_copy(start_, p, newStart);
		newFinish = SimSTL::uninitialized_copy(first, last, newFinish);
		auto res = newFinish;
		newFinish = SimSTL::uninitialized_copy(p, finish_, newFinish);
	...
	string::iterator string::insert(iterator p, InputIterator first, InputIterator last){
		...
		if (lengthOfInsert <= lengthOfLeft){
			...
			SimSTL::uninitialized_copy(first, last, p);

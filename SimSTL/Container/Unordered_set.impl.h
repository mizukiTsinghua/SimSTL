...
namespace SimSTL{
	...
	bool Unordered_set<Key, Hash, KeyEqual, Allocator>::has_key(const key_type& key){
		...
		return SimSTL::find_if(list.begin(), list.end(), pred) != list.end();
	}
	template<class Key, class Hash, class KeyEqual, class Allocator>
	typename Unordered_set<Key, Hash, KeyEqual, Allocator>::size_type
	...
	template<class Key, class Hash, class KeyEqual, class Allocator>
	SimSTL::pair<typename Unordered_set<Key, Hash, KeyEqual, Allocator>::iterator, bool> 
		...
		if (!has_key(val)){
			...
			++size_;
			return SimSTL::pair<iterator, bool>(iterator(index, buckets_[index].begin(), this), true);
		}
		return SimSTL::pair<iterator, bool>(end(), false);
	}
	template<class Key, class Hash, class KeyEqual, class Allocator>
	template<class InputIterator>
	...
	void Unordered_set<Key, Hash, KeyEqual, Allocator>::rehash(size_type n){
		...
		SimSTL::swap(*this, temp);
	}
	template<class Key, class Hash, class KeyEqual, class Allocator>
	void swap(Unordered_set<Key, Hash, KeyEqual, Allocator>& lhs,
		Unordered_set<Key, Hash, KeyEqual, Allocator>& rhs){
		SimSTL::swap(lhs.buckets_, rhs.buckets_);
		SimSTL::swap(lhs.size_, rhs.size_);
		SimSTL::swap(lhs.max_load_factor_, rhs.max_load_factor_);
	}
}
